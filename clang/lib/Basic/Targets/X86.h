...
public:
  ...
  LangOptions::FPEvalMethodKind getFPEvalMethod() const override {
    // X87 computes at 80 bits, but the backend rounds every result back to its
    // own type unless -fexcess-precision=fast asked it not to.
  ...
  // `source` needs every x87 result rounded to its type, which is what the
  // backend does unless -fexcess-precision=fast turned it off.
